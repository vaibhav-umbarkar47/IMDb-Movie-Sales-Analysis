<img width="1203" height="638" alt="image" src="https://github.com/user-attachments/assets/04a09067-1171-46c9-8c5c-bf4888fbce93" />

🎬 ****IMDb Movie Sales Analysis – Power BI Project****



This project presents an in-depth analysis of IMDb movie data using Power BI.
The goal is to identify key trends in movie performance, including revenue, ratings, genres, and runtime impact.
All insights are based on a cleaned dataset of 1,000 movies.

---

📌 **Project Overview**

In today’s competitive movie industry, understanding what makes a movie successful is crucial.
Using IMDb data, this project explores:

🎥 Which directors generate the highest revenue
⭐ Which genres receive the best audience ratings
⏱️ How runtime affects ratings and revenue
📅 Trends in movie releases over the years

This analysis helps uncover audience preferences and factors that drive movie success.

---

🎯 **Business Objectives**

🎬 Identify the top-earning director
🎭 Determine the genre with the highest average audience rating
⏳ Understand the impact of movie runtime on ratings & revenue
📈 Analyse yearly trends in movie releases and revenue

---

🛠️ **Approach**

📊 Analyzed total revenue to find the top directors
⭐ Calculated average ratings to determine best-performing genres
⏱️ Evaluated the effect of runtime on ratings & revenue
🖥️ Designed a Power BI dashboard to present insights clearly

---

🧹 **Data Preparation**

📁 Dataset Used: IMDb Movie Dataset
📌 Size: 1,000 Records • 12 Columns

---

🔧 **Cleaning Steps**

✔️ Loaded dataset into Power BI
✔️ Handled missing values (Revenue Millions & Metascore)
✔️ Converted data types:

💰 *Revenue & Metascore → Decimal number*

📆 *Year & ⏱️ Runtime → Whole number*
✔️ *Split multi-genre fields*

Example: Action|Adventure|Sci-Fi → Action, Adventure, Sci-Fi

---

📊 **Key Insights**
⭐ Movie Release Trends

📅 2016 had the highest number of releases (297)
⬇️ From 2008 to 2015, movie releases gradually decreased
📈 Revenue increased consistently from 2011 to 2016, peaking at 11B

---

⭐ **Genres & Ratings**

🎨 Animation & 🧑‍🎨 Biography → Highest average rating: 7.3
🎭 Drama → Strong performance with a rating of 7.0

---

⭐ **Top Directors by Revenue**:

🎬 J.J. Abrams → Highest revenue (~1.7B)
🎥 David Yates → High revenue
🎞️ Christopher Nolan → Strong performer

---

⭐ **Impact of Runtime**:

⏱️ Ideal runtime for best-performing movies: 120–180 minutes
📈 Longer runtime increases both ratings and revenue
⚠️ Movies over 180 minutes show mixed performance

---

📈 **Dashboard Link**:
<img width="900" height="400" alt="Screenshot 2025-10-12 002618" src="https://github.com/user-attachments/assets/629e74ed-d5f1-4505-8904-ec915dcc473b" />


🔗 View Complete Power BI Dashboard:
https://app.powerbi.com/groups/me/reports/8e95c9ea-3a37-485f-bb63-1750817e7d08/eb72fb596c0210a16297?experience=power-bi

---

🖼️ **Project Files**:

📊 Dataset

📝 Project Presentation (PPT/PDF)

---

📌 **Conclusion**

This analysis clearly shows that:

✔️ Genres, director performance, and runtime significantly influence movie success
✔️ Animation & Biography are the highest-rated genres
✔️ J.J. Abrams leads in total revenue
✔️ Movies around 2.5 to 3 hours perform best

This project demonstrates how data analytics can deliver powerful insights into the movie industry.

---

🙌 **Connect With Me**

🔗 LinkedIn:
https://www.linkedin.com/in/vaibhav-umbarkar-023667324/

🔗 GitHub:
https://github.com/vaibhav-umbarkar47
